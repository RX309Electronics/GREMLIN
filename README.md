...
8. (Optional) Try it out by executing './Test.sh', which will launch qemu and boot up your distro. You can select if you want to boot up as a RAMDISK or a disk.img file. 
...
# Hardware support
Curently very mixed results but will do some more testing. Works atleast in QEMU Vm, and ramdisk mostly works on real hardware. Kernel tweaking and fine tuning is still not done. Audio, graphics and networking seems to work on my test system, the dell latitude 7290, but needs further testing on other hw combinations. 
